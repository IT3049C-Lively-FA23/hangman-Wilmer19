class Hangman {
  ...
  guess(letter) {
    ...
    if (!letter){
      throw new Error("Must provide a letter.");
    }
    if (!/[a-zA-Z]/.test(letter)){
      throw new Error("Invalid Input. Must enter a letter");
    }
    if (letter.length !== 1){
      throw new Error("Must enter only one letter");
    }
  ...
  checkWin() {
    ...
    const remainingUnknowns = word.filter(letter => !
      guesses.includes(letter)).length;
